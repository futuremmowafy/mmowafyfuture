      ...
      if (allDevs && allDevs.length > 0) {
        ...
        for (const candidateDev of allDevs) {
          ...
          for (const s of serialsToCheck) {
            ...
            if (Math.abs(cleanLower.length - s.length) === 1) {
    ...
    case 'device_intake': {
      ...
      if (isBatch) {
        for (let i = 0; i < batchItems.length; i++) {
          ...
          if (it.indoor_serial) {
            const existingCheck = await findDeviceInWarehouse(it.indoor_serial, false);
            if (existingCheck.found && existingCheck.exact && existingCheck.isAvailable) {
          ...
          if (it.outdoor_serial) {
            const existingOutCheck = await findDeviceInWarehouse(it.outdoor_serial, false);
            if (existingOutCheck.found && existingOutCheck.exact && existingOutCheck.isAvailable) {
              return ctx.reply(`⚠️ <b>تنبيه: سيريال الكباس للجهاز (${i + 1}) [<code>${it.outdoor_serial}</code>] موجود بالفعل في المخزن كـ متاح!</b>\nيرجى مراجعة السيريال لتفادي التكرار.`, { parse_mode: 'HTML' });
            }
          }
        }
        ...
        if (workingData.indoor_serial) {
          const existingCheck = await findDeviceInWarehouse(workingData.indoor_serial, false);
          if (existingCheck.found && existingCheck.exact && existingCheck.isAvailable) {
        ...
        if (workingData.outdoor_serial) {
          const existingOutCheck = await findDeviceInWarehouse(workingData.outdoor_serial, false);
          if (existingOutCheck.found && existingOutCheck.exact && existingOutCheck.isAvailable) {
            return ctx.reply(`⚠️ <b>تنبيه: سيريال الكباس [<code>${workingData.outdoor_serial}</code>] موجود بالفعل في المخزن كـ متاح!</b>\nيرجى مراجعة السيريال لتفادي التكرار.`, { parse_mode: 'HTML' });
          }
        }
      }
