...
function hasAcSpecifier(brandStr) {
  ...
  const b = String(brandStr).toLowerCase();
  return b.includes('بارد') || b.includes('ساخن') || b.includes('انفرتر') || b.includes('أنفرتر') || b.includes('عادي') || b.includes('كونسيلد') || b.includes('شباك') || b.includes('فري ستاند') || b.includes('سقف');
}
...
async function findDeviceInWarehouse(serial, allowFuzzy = true) {
  ...
  // 3. Try inside installment_notes (outdoor serial tag or substring)
  if (!dev) {
    const { data: devNotes } = await supabase
      .from('devices')
      .select('*, suppliers(name)')
      .or(`installment_notes.ilike.%[سيريال كباس: ${cleanSerial}]%,installment_notes.ilike.%[الوحدة الخارجية: ${cleanSerial}]%,installment_notes.ilike.%${cleanSerial}%`)
      .limit(1)
      .maybeSingle();
    dev = devNotes;
  }
  ...
  // 4. Smart Fuzzy Matching against Warehouse Devices (Fixes voice transcription digit-stutter / repeating numbers)
  if (allowFuzzy && cleanSerial.length >= 5) {
    try {
      ...
      if (allDevs && allDevs.length > 0) {
        ...
        for (const candidateDev of allDevs) {
          ...
          if (candidateDev.serial_number) serialsToCheck.push(String(candidateDev.serial_number).trim().toLowerCase());
          if (candidateDev.outdoor_serial) serialsToCheck.push(String(candidateDev.outdoor_serial).trim().toLowerCase());
          
          const noteMatch = (candidateDev.installment_notes || '').match(/\[سيريال كباس:\s*([^\]]+)\]/);
          if (noteMatch && noteMatch[1]) {
            serialsToCheck.push(noteMatch[1].trim().toLowerCase());
          }

          for (const s of serialsToCheck) {
            const dist = getLevenshteinDistance(cleanLower, s);
            // Allow distance <= 1 for 6+ chars, distance <= 2 for 8+ chars when length differs by <= 2
            const maxAllowed = cleanLower.length >= 8 ? (dist <= 1 || (dist <= 2 && Math.abs(cleanLower.length - s.length) <= 2)) : (dist <= 1);
            if (maxAllowed && dist < minDistance) {
              minDistance = dist;
...
function normalizeBatchItems(workingData) {
  ...
  if (workingData.brand) workingData.brand = cleanTextString(workingData.brand);
  ...
  if (items.length > 0) {
    items.forEach(it => {
      ...
      if (it.brand) it.brand = cleanTextString(it.brand);
    });
  }
    ...
    case 'device_intake': {
      ...
      if (isBatch) {
        for (let i = 0; i < batchItems.length; i++) {
          ...
          if (it.indoor_serial) {
            const existingCheck = await findDeviceInWarehouse(it.indoor_serial);
            if (existingCheck.found && existingCheck.isAvailable) {
              return ctx.reply(`⚠️ <b>تنبيه: سيريال الفانة للجهاز (${i + 1}) [<code>${it.indoor_serial}</code>] موجود بالفعل في المخزن كـ متاح!</b>\nيرجى مراجعة السيريال لتفادي التكرار.`, { parse_mode: 'HTML' });
            }
          }
        }
        ...
        if (workingData.indoor_serial) {
          const existingCheck = await findDeviceInWarehouse(workingData.indoor_serial);
          if (existingCheck.found && existingCheck.isAvailable) {
            return ctx.reply(`⚠️ <b>تنبيه: سيريال الفانة [<code>${workingData.indoor_serial}</code>] موجود بالفعل في المخزن كـ متاح!</b>\nيرجى مراجعة السيريال لتفادي التكرار.`, { parse_mode: 'HTML' });
          }
        }
      }
...
export function registerHandlers(bot) {
  ...
  bot.start(async (ctx) => {
    ...
    const keyboard = {
      inline_keyboard: [
        [
          { text: '📱 فتح لوحة التحكم التفاعلية (Mini App)', web_app: { url: 'https://futureairpro.onrender.com/' } }
        ],
        [
  ...
  bot.on(['photo', 'document'], async (ctx) => {
    ...
    try {
      ...
      } else if (lowerCap.includes('صرف') || lowerCap.includes('تسليم')) {
        docMarker = '__delivery_note__';
        detectedLabel = 'إذن صرف مبيعات';
      } else if (lowerCap.includes('استلام') || lowerCap.includes('توريد')) {
